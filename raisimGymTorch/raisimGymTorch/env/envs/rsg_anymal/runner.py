...
logging_date = datetime.datetime.now().strftime("%m-%d-%H-%M-%S")
...
env = VecEnv(RaisimGymEnv(home_path + "/rsc", dump(cfg['environment'], Dumper=RoundTripDumper)), logging_date)
env.seed(cfg['seed'])
    ...
    if update % cfg['environment']['eval_every_n'] == 0:
        ...
        for step in range(n_steps):
            with torch.no_grad():
                ...
                reward, dones = env.step(action.cpu().detach().numpy(), True, update)
