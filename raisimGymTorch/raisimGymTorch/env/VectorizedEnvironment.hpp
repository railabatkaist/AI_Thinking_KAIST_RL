...
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      perAgentStep(i, action, reward, done, logging, iter);
  }
  ...
  void setLoggingDate(const std::string& loggingDate) {
      environments_[0]->setLoggingDate(loggingDate);
  }
...
#pragma omp parallel for schedule(auto)
  ...
  inline void perAgentStep(int agentId,
                           ...
                           bool logging,
                           int iter) {
    reward[agentId] = environments_[agentId]->step(action.row(agentId), logging, iter);
